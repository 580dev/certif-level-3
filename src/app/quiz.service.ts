...
import {Category, Difficulty, ApiQuestion, Question, Results, CategoriesName, entertainmentCategory, scienceCategory, TypeCategory} from './data.models';
...
export class QuizService {
  ...
  getAllCategories(): Observable<Category[]> {
    return this.http.get<{ trivia_categories: Category[] }>(this.API_URL + "api_category.php").pipe(
      map(res => {
        let categories : Category[] = res.trivia_categories
        categories.map((category: Category) => { 
          let categoryNames : string[] = category.name.split(": ")
          category.typeCategory = categoryNames.length === 1 ? undefined : categoryNames[0] as TypeCategory
          category.name = categoryNames.length === 1 ? categoryNames[0] : categoryNames[1]
          category.isSubCategory = categoryNames.length > 1
          return category
        })
       
        categories.push(entertainmentCategory, scienceCategory)
        return categories
      })
    );
  }
